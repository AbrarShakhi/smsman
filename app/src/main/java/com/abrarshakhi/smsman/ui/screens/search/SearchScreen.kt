...
fun SearchScreen(
    ...
    Scaffold(
        ...
        Box(modifier = Modifier.fillMaxSize().padding(padding)) {
            when {
                state.isSearching -> CenterLoading()
                state.query.isEmpty() -> SearchPrompt(prompt = "Type to search across all conversations.")
                state.hits.isEmpty() && state.hasSearched -> SearchPrompt(
                    prompt = "No matches for \"${state.query}\".",
                )
                state.hits.isEmpty() -> SearchPrompt(prompt = "Keep typing…")
                else -> SearchResults(
                    hits = state.hits,
                    query = state.query,
...
@Composable
private fun SearchPrompt(prompt: String) {
...
private fun SearchResults(
    ...
    LazyColumn(modifier = Modifier.fillMaxSize()) {
        items(hits, key = { it.message.id }) { hit ->
            SearchResultRow(hit = hit, query = query, onClick = { onClick(hit.conversation.threadId) })
...
private fun SearchResultRow(
    ...
    Row(
        ...
        ContactAvatar(
            contact = hit.contact,
            fallbackLabel = hit.conversation.recipientAddresses.firstOrNull().orEmpty(),
            size = 40.dp,
        )
        Spacer(Modifier.width(12.dp))
        Column(modifier = Modifier.weight(1f)) {
            Text(
                text = hit.contact?.displayLabel ?: hit.conversation.recipientAddresses.firstOrNull().orEmpty(),
        ...
        Text(
            text = formatRelativeListTime(hit.message.date),
